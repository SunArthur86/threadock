...
pub fn parse_session(db_path: impl AsRef<Path>, session_id: &str) -> AdapterResult<RawConversation> {
    ...
    // 1. 会话元信息（含时间戳）
    let (title, _directory, model, time_created, time_updated): (
        String, String, Option<String>, i64, i64,
    ) = conn
        .query_row(
            "SELECT title, directory, NULL, time_created, time_updated FROM session WHERE id = ?1",
            params![session_id],
            |r| Ok((r.get(0)?, r.get(1)?, r.get::<_, Option<String>>(2)?, r.get(3)?, r.get(4)?)),
        )
        .map_err(|e| match e {
...
mod tests {
    ...
    fn filters_system_reminder() {
        ...
        conn.execute("INSERT INTO session VALUES ('s','t','d',0,0)", []).unwrap();
